...
/**
 * Diabolist (White) — 白い悪魔魔法師（困難モード地下城）
 * 赤変種と全く同じステータス、袍子の色のみ異なる
 */
public class DiabolistWhiteEntity extends Skeleton {
    private boolean hasInitializedEquipment = false;

    public DiabolistWhiteEntity(EntityType<? extends Skeleton> type, Level level) {
        super(type, level);
    }
    ...
    private void setupEquipment() {
        ...
        legsDisplay.putInt("color", whiteRobeColor);
    ...
    public void shootInfernoBolt(LivingEntity target) {
        ...
        bolt.setDeltaMovement(direction.scale(0.7));
    ...
    static class TeleportAndShootGoal extends Goal {
        private final DiabolistWhiteEntity diabolist;
        private LivingEntity target;
        private int actionTimer;
        private int shotsFired;

        private static final int SHOTS_PER_BURST     = 3;
        private static final int TICKS_BETWEEN_SHOTS = 20;
        private static final int COOLDOWN_AFTER_BURST = 50;

        public TeleportAndShootGoal(DiabolistWhiteEntity diabolist) {
        ...
        @Override
        public boolean canContinueToUse() { return this.target != null && this.target.isAlive(); }
